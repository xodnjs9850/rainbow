T.test("s2: 카드 3종 선택 → AI 그림 만들기 → 3안 + 필터 로그 → 선택·이름 → 완료", function () {
  ...
  T.eq(root.querySelectorAll(".tab").length, 0, "탭 없음"); T.has(root, "카드 고르기");
  T.eq(root.querySelectorAll(".opt").length, 3);
...
T.test("s2: 이미지 로드 실패 시 SVG 캐릭터로 대체된다", function () {
  ...
  T.ok(svg.outerHTML.indexOf("#3B82F6") > 0, "선택한 색 반영"); T.ok(!root.querySelector('.char[data-i="1"] img'), "img 제거");
});
