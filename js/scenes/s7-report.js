(function () {
  ...
  // k = [라벨(자동 escape), 값 HTML(호출부에서 escape), 강조 클래스]
  ...
  function paint() {
    ...
    LB.crumb("보고 › " + r.id); LB.badge(su.alerts);
    root.innerHTML = '<div class="head"><h2>세션 종료와 보고</h2><span class="badge gray">' + esc(r.id) + '</span>'
      ...
      + kpi([["세션 시간", esc(su.duration)], ["인원 확인", esc(su.checkTime), "info"], ["경보", esc(su.alerts + "건"), "warn"],
             ["구역이탈 · SOS", esc(su.exits + " · " + su.sos)], ["오탐", esc(su.falsePos + "건")], ["평균 대응", esc(su.avgResponse), "info"]], "three")
      ...
      + '<p style="margin-top:8px">이번 세션에서는 비허용 구역 우세로 확인 필요 알림이 ' + esc(su.exits) + '건 발생했고 ' + esc(t) + '가 ' + esc(e1.action) + '로 처리했다. SOS ' + esc(su.sos) + '건은 버튼 ' + esc(e2.action) + '으로 기록되었다. ' + esc(e1.zone) + ' 인접 구간에서 이탈 알림이 반복될 수 있어 다음 세션에서는 인솔자 배치를 검토할 것을 제안한다.</p></div>'
      ...
      + kpi([["활성 학급", esc(String(m.classes))], ["세션", esc(String(m.sessions))], ["30분 수용성", esc(m.acceptance), "info"],
